...
try:
    ...
    print(f"🚀 חומרה זוהתה: {torch.cuda.get_device_name(0) if DEVICE == 'cuda' else 'CPU'}")
except ImportError:
    DEVICE = "cpu"
...
class Task:
    task_id: str
    song_name: str
    status: str  # ⏳ waiting, 📥 downloading, 🎚️ separating, 📝 transcribing, 🎬 rendering, ✅ done, ❌ error
...
class TaskManager:
    ...
    def update_task(self, task_id, status=None, progress=None):
        with self.lock:
            if task_id in self.tasks:
                ...
                if DEVICE == "cuda":
                    try:
                        ...
                        self.tasks[task_id].vram_used = f"{used_gb:.1f}GB"
                    except: pass
    ...
    def get_dataframe(self):
        with self.lock:
            ...
            for tid, task in sorted(self.tasks.items()):
                ...
                data.append({
                    "ID": tid,
                    "שיר": task.song_name[:30],
                    "סטטוס": task.status,
                    "התקדמות": f"{int(task.progress)}%",
                    "VRAM": task.vram_used,
                    "זמן (s)": elapsed
                })
            return pd.DataFrame(data) if data else pd.DataFrame(columns=["ID", "שיר", "סטטוס", "התקדמות", "VRAM", "זמן (s)"])
...
class ResourceManager:
    def __init__(self, max_concurrent_heavy=2):
        ...
        self.active_tasks = 0
        self.lock = threading.Lock()
        self.current_status = "🟢 פנוי להוראות"
    ...
    def get_status(self):
        ...
        if DEVICE == "cuda":
            try:
                ...
                mem_info = f"(VRAM: {free_gb:.1f}GB/{total_gb:.1f}GB)"
                if free_gb < 4: return f"🔴 עומס גבוה מאוד {mem_info} - ממתין..."
            except: pass
        
        if active == 0: 
            self.current_status = f"🟢 פנוי להוראות {mem_info}"
        elif active == 1: 
            self.current_status = f"🟡 עובד (משימה 1) {mem_info}"
        elif active >= 2: 
            self.current_status = f"🔴 עמוס (2 משימות) {mem_info}"
    ...
    def get_df(self):
        return task_manager.get_dataframe()

mm = ResourceManager(max_concurrent_heavy=2)

# ==========================================
# חלק 2: מנוע העיבוד (Backend)
# ==========================================
...
class BackendProcessor:
    ...
    def get_video_info(self, query, current_logs):
        ...
        if not query.startswith("http"):
            ...
            self.log(f"🔎 מחפש: '{query}'...", current_logs)
        else:
            self.log(f"🔎 מנתח: '{query}'...", current_logs)
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                self.log(f"✅ זוהה: {clean_title}", current_logs)
    ...
    def download_video(self, video_info, current_logs):
        ...
        if os.path.exists(target_path):
            self.log(f"✅ קובץ מקור קיים.", current_logs)
            return target_path
        ...
        try:
            ...
            self.log(f"❌ שגיאה בהורדה: {e}", current_logs); return None
            ...
            try:
                ...
                if os.path.exists(temp_check):
                    ...
                    if abs(src_size - pb_size) < 1024: 
                        self.log("⚠️ זוהה פלייבק פגום. מבצע הפרדה מחדש...", current_logs)
                        should_process = True
                    else:
                        self.log(f"✅ קבצי הפרדה תקינים קיימים.", current_logs)
        ...
        with mm.semaphore:
            ...
            try:
                ...
                if save_4 and os.path.exists(input_wav):
                    self.log(f"🎚️ מפריד ל-4 ערוצים...", current_logs)
    ...
    def transcribe_audio(self, audio_path, output_folder, title, current_logs, lang="he"):
        ...
        if os.path.exists(final_ass):
            self.log(f"✅ תמלול קיים.", current_logs)
            return final_ass
        ...
        with mm.semaphore:
            mm.start_task()
            try:
                self.log("📝 מתחיל תמלול...", current_logs)
                self.cleanup_resources()
    ...
    def render_video(self, video_path, audio_path, ass_path, video_info, current_logs, use_bidi=False):
        ...
        if os.path.exists(final_video):
            self.log(f"✅ סרטון קיים.", current_logs)
            return final_video

        self.log(f"🎬 מרנדר...", current_logs)
    ...
    def process_song_pipeline(self, query, lang, save_4_stems, use_bidi, force_process):
        task_id = task_manager.add_task(query)
        current_logs = []
        try:
            task_manager.update_task(task_id, "📥", 10)
            info = self.get_video_info(query, current_logs)
            if not info: 
                task_manager.update_task(task_id, "❌", 0)
                return None, "\n".join(current_logs)
            
            task_manager.update_task(task_id, "📥", 30)
            video_path = self.download_video(info, current_logs)
            if not video_path: 
                task_manager.update_task(task_id, "❌", 0)
                return None, "\n".join(current_logs)
            
            task_manager.update_task(task_id, "🎚️", 40)
            ...
            task_manager.update_task(task_id, "📝", 65)
            ...
            task_manager.update_task(task_id, "🎬", 85)
            final = self.render_video(video_path, playback, ass_path, info, current_logs, use_bidi)
            if final:
                task_manager.update_task(task_id, "✅", 100)
            else:
                task_manager.update_task(task_id, "❌", 0)
            return final, "\n".join(current_logs)
        except Exception as e:
            task_manager.update_task(task_id, "❌", 0)
            self.log(f"❌ שגיאה: {e}", current_logs)
            return None, "\n".join(current_logs)
        finally:
