    ...
    def get_status(self):
        ...
        if DEVICE == "cuda":
            try:
                ...
                mem_info = f"(VRAM פנוי: {free_gb:.1f}GB)"
                if free_gb < 4: return f"🔴 עומס גבוה מאוד {mem_info} - ממתין..."
            except: pass
        
        if active == 0: return f"🟢 פנוי להוראות {mem_info}"
        if active == 1: return f"🟢 עובד (משימה 1) {mem_info}"
        if active >= 2: return f"🟠 עמוס (2 משימות) {mem_info}"
        return "Unknown"
...
class BackendProcessor:
    ...
    def process_song_pipeline(self, query, lang, save_4_stems, use_bidi, force_process):
        ...
        if final: gr.Info(f"✅ הסתיים: {os.path.basename(final)}")
        return final, "\n".join(current_logs)
...
with gr.Blocks(title="Karaoke V64 Notifications", theme=gr.themes.Soft()) as app:
    gr.Markdown(f"# 🎤 Karaoke Studio Pro V64 ({DEVICE.upper()})")
    
    status_label = gr.Label(value="מערכת מוכנה", label="System Status")
    timer = gr.Timer(2)
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🛠️ כלים מתקדמים"):
            with gr.Tabs():
                ...
                with gr.Tab("🗣️ תמלול"):
                    tr_u = gr.Textbox(label="URL"); tr_f = gr.File(label="או קובץ")
                    tr_lang = gr.Dropdown(["he", "en"], value="he"); tr_b = gr.Button("תמלל")
                    tr_o = gr.File(label="ASS"); tr_txt = gr.TextArea(label="תוכן"); tr_l = gr.TextArea(label="לוגים")
